### Node 클래스 구현해서 BinaryTree 구현해서 풀면 통과, List index로 접근하면 시간초과, 런타임에러도 남
### 런타임에러는 잘못 구현한게 있겠지만 시간초과 났다는건 편향이진트리가 있었다거나... 리스트 다 돌기엔 극단적인 테케가 있는거 같음.
### @ 파이썬 재귀제한은 디폴트가 1000회, 이 문제는 이 제한을 늘려줘야 통과할 수 있었음(sys.setrecursionlimit(제한할 수))
